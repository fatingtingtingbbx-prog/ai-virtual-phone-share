...
**全部资源统一放在 `资源/` 目录下**（根目录其它文件夹是服务代码，与资源无关）。
...
└── 预设/
    ...
    │   ├── .title            ← 自动生成：带贴纸/排版的原始标题（文件夹名是它的安全化版本）
    │   ├── .author           ← 自动生成：投稿人名字（详情页展示），可不填
    │   ├── .avatar.png       ← 自动生成：投稿人头像（64×64），可不填
    │   └── .owner            ← 自动生成：投稿者删除/编辑凭证的哈希，勿动
...
`_flowers.json` 是送花计数表（{资源路径: 花数}），由上传服务的机器人维护，同样**不要手改**。
...
接口动作：`upload`（开 PR 待审）、`edit`（作者持凭证改自己的资源，立即生效）、
`delete`（作者自助下架）、`flower`（送花计数）。

`_config.json`（仓库根目录）里的 `autoApprove: true` 表示开启自动审核：
投稿一到就自动合并上架，不再排队等人工。这个文件只能由有仓库写权限的人
修改（App 的管理中心里有开关），GitHub 会拒绝其他人的写入。

安全设计：投稿只会开 PR（待审核），永远不直接改 main；管理员 merge 才上架。
编辑与删除需要凭证哈希比对通过（凭证本体只在投稿者设备上），且只能动自己那个资源文件夹。
含单文件/总量体积限制与 IP 频控。
