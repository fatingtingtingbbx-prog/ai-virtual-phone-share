...
//   按 HIM 四性格排列：①温暖(钢琴阳光房) ②沉静(实验室沉思) ③冷傲(书房看书+猫) ④热烈(西装办公+雕像)。
const VIDEO_OPTIONS = [
  { label: "① 温暖·钢琴阳光", value: "https://file.zhuyitai.com/feedback/202608/c1f6af0c8adfd88db08216ed741aeec1.mp4" },
  { label: "② 沉静·实验室",   value: "https://cn.oilgasgpts.com/resource/profile/2026/08/19/RM1eBAP4_20260819203213A458.mp4" },
  { label: "③ 冷傲·书房猫",   value: "https://nos.netease.com/ysf/f8f20d99a451179a66c2ccfc7881afbd.mp4" },
  { label: "④ 热烈·西装雕像", value: "https://www.yn12377.cn/jubao/upload/smjb/2026/08/19/a5cc571639e146e1b60c5234e536adf2.mp4" },
];
...
export default {
  ...
  setup(ctx) {
    ...
    // 说话节奏：把「说话中」切成 「一段波澜 → 平 → 一段波澜 → 平」 的自然语句节奏，
    // 段与段之间用缓动平滑过渡，避免二值 on/off 在一帧里生硬跳变。
    let talkSeg = "speak";     // 当前段："speak"=波澜  "flat"=平
    let talkStart = 0;         // 当前段起始时间戳(ms)
    let talkDur = 1;           // 当前段时长(ms)
    let talkHold = 0.7;        // 当前波澜段的基准幅度（每段略有差异）
    let waveLastT = 0;         // 上一帧时间戳，用于帧率无关的平滑
    let waveActiveUntil = 0;   // 去抖：宿主 data-speaking 瞬断时，把「说话中」延续到此时刻
    ...
    const drawWave = () => {
      ...
      // 去抖：宿主 data-speaking 在说话间隙会偶发瞬断一两帧，直接切断会让波澜「啪」地收一下；
      // 只有持续静默超过 220ms 才真正转入平，瞬断被吸收，波澜连续不卡。
      let active;
      if (waveSpeaking) { waveActiveUntil = now + 220; active = true; }
      else { active = now < waveActiveUntil; }
      let targetAmp;
      if (!active) {
        targetAmp = 0;                                   // 持续静默 / 未接通：彻底平
      } else {
        if (now - talkStart >= talkDur) {                // 当前段到时 → 切到下一段
          if (talkSeg === "speak") {
            talkSeg = "flat";
            talkDur = 300 + Math.random() * 700;         // 平 0.3~1.0s（停顿/换气）
          } else {
            talkSeg = "speak";
            talkDur = 700 + Math.random() * 1500;        // 波澜 0.7~2.2s（一段话）
            talkHold = 0.55 + Math.random() * 0.35;      // 每段波澜基准幅度略有差异
          }
          talkStart = now;
        }
        if (talkSeg === "flat") {
          targetAmp = 0;                                 // 平：只剩基线细线
        } else {
          const p = (now - talkStart) / talkDur;         // 段内进度 0..1
          // Hann 窗：段首、段尾都平滑落到 0 → 波澜自然升起又落下，没有突兀的 pop
          const win = Math.pow(0.5 - 0.5 * Math.cos(2 * Math.PI * p), 0.85);
          const wob = 0.85 + 0.15 * Math.sin(now * 0.005 + wave.a1 * 6); // 段内轻微起伏
          targetAmp = talkHold * win * wob;
        }
      }
