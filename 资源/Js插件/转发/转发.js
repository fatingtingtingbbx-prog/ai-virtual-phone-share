export default {
  manifest: {
    ...
    version: "7.1.0",
    author: "倚枫歌",
    description: "最终完美版：发送秒显、彻底透明化专属气泡。AI 能够完全看懂聊天记录内容（包含精确时间和图片/语音/红包标注）。",
  ...
  setup(ctx) {
    ctx.ui.injectCSS(`
      ...
      /* 🔥 纯 CSS 隐身衣，安全隐藏长文本而不破坏 DOM */
      .wcf-magic-bubble { background: transparent !important; box-shadow: none !important; border: none !important; padding: 0 !important; }
      .wcf-magic-bubble::before, .wcf-magic-bubble::after { display: none !important; content: none !important; }
      .wcf-hide-text { font-size: 0 !important; color: transparent !important; margin: 0 !important; padding: 0 !important; line-height: 0 !important; opacity: 0 !important; pointer-events: none !important; }
      
      .wcf-plugin-container { font-size:14px !important; line-height:normal !important; color:#1f2b22 !important; display:block; text-align:left; opacity: 1 !important; pointer-events: auto !important; }
    ...
    function renderForwardCard(el, forwardData) {
      ...
      if (!container) {
        ...
        container.className = 'wcf-plugin-container';
        ...
        try {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            if (bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent') {
              curr.classList.add('wcf-magic-bubble');
              // 把气泡里的原生文字段落全部变成隐形（但保留在 DOM 树中供 AI 读取）
              Array.from(curr.childNodes).forEach(n => {
                if (n !== el && n.nodeType === 1) n.classList.add('wcf-hide-text');
              });
              break; 
            }
        ...
        filteredSessions.forEach(session => {
          ...
          item.addEventListener('click', async () => {
            try {
              ...
              // 🔥 核弹级改动：带时间戳的隐身纯文本记录（供 AI 阅读）
              let aiReadableContent = `[你向对方发送了一份合并转发的聊天记录：${dynamicTitle}]\n`;
              snapshots.forEach(m => {
                  let role = m._displayRole || '未知';
                  let text = m.content ? String(m.content).trim() : '';
                  let timeStr = m.createdAt ? formatTime(m.createdAt) : '';
                  
                  let mediaDesc = '';
                  if (m.mediaType) {
                      const t = m.mediaType.toLowerCase();
                      if (t.includes('image')) mediaDesc = '[图片]';
                      else if (t.includes('audio') || t.includes('voice')) mediaDesc = '[语音]';
                      else if (t.includes('video')) mediaDesc = '[视频]';
                      else if (t.includes('redpack') || t.includes('hongbao') || (m.mediaData && m.mediaData.amount)) mediaDesc = '[红包]';
                      else if (t.includes('plugin:')) mediaDesc = '[应用卡片]';
                      else mediaDesc = '[多媒体文件]';
                  }
                  
                  let finalLine = text;
                  if (mediaDesc) finalLine = text ? `${text} ${mediaDesc}` : mediaDesc;
                  if (!finalLine) finalLine = '[未知内容]';
                  
                  let timePrefix = timeStr ? `[${timeStr}] ` : '';
                  aiReadableContent += `${timePrefix}${role}: ${finalLine}\n`;
              });

              await ctx.data.messages.push({
                sessionId: session.id,
                role: 'user',
                content: aiReadableContent.trim(), // 塞入长文本，UI端会被隐身
    ...
    function openSingleTargetSelector(msg, sourceSession) {
      ctx.ui.openModal((modalEl, { close }) => {
        ...
        filteredSessions.forEach(session => {
          ...
          item.addEventListener('click', async () => {
            try {
              let pushContent = msg.content || '';
              if (msg.mediaType) {
                  const t = msg.mediaType.toLowerCase();
                  if (t.includes('redpack') || t.includes('hongbao') || (msg.mediaData && msg.mediaData.amount)) {
                      pushContent = pushContent ? pushContent + ' [发给对方一个红包]' : '[发给对方一个红包]';
                  } else if (t.includes('audio') || t.includes('voice')) {
                      pushContent = pushContent ? pushContent + ' [语音]' : '[发给对方一段语音]';
                  } else if (t.includes('image') || t.includes('video')) {
                      pushContent = pushContent ? pushContent + ' [图片/视频]' : '[发给对方一张图片/视频]';
                  }
              }
              
              await ctx.data.messages.push({
                sessionId: session.id, role: 'user', content: pushContent, mediaType: msg.mediaType, mediaData: msg.mediaData
              });
