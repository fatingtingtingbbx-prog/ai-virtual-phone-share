...
export default {
  manifest: {
    ...
    version: "2.0.0",
    author: "小坊",
    description: "文艺优雅版：支持在插件设置中集中管理各角色语言与翻译开关。AI 翻译支持“文艺优雅”与“自然口语”双风格，极速且细腻。",
    ...
    settings: [
      ...
      },
      {
        ...
        default: "llm",
        options: [
          { value: "llm", label: "AI 智能意译 (更通顺、有人情味)" },
          { value: "google", label: "谷歌翻译 (免费极速)" }
        ]
      },
      {
        key: "targetLang",
  ...
  setup(ctx) {
    ...
    async function translateToChinese(text) {
      const engine = ctx.system.settings.get("engine") || "llm";
      const targetLang = ctx.system.settings.get("targetLang") || "中文";
      const style = ctx.system.settings.get("transStyle") || "literary";

      if (text.startsWith("mediastore://")) return "[多媒体内容]";
      ...
      if (engine === "llm") {
        try {
          ...
          if (style === "literary") {
            // 文艺优雅风格 Prompt
            prompt = `你是一位极具文学素养和情感温度的同声传译家。请将以下外语社交聊天内容，意译为极具文艺感、优雅且细腻的${targetLang}。
在保留原文情感内核的前提下，用词要雅致自然、优美动人，带有微妙的故事感和诗意，严禁机械直译。直接输出翻译结果，不要有任何多余的评价：\n\n${text}`;
          } else {
            // 普通口语风格 Prompt
            prompt = `将以下外语聊天意译为自然、契合日常聊天语境的口语化${targetLang}。严禁翻译腔，直接输出结果：\n\n${text}`;
          }

          const res = await ctx.ai.chat({
            prompt,
            temperature: 0.4, // 文艺风格需要稍微多一点情感张力
            maxTokens: 150
          });
          return res.trim();
        } catch (err) {
          ctx.system.log("AI 翻译超时，降级到谷歌极速翻译:", err);
        }
      }
    ...
    ctx.ui.slot("message.footer", (el, props) => {
      ...
      if (!msg || !msg.content || msg.role === "system") return;

      const charId = getCharacterIdForSession(props.sessionId);
      if (!charId) return;

      const isEnabled = ctx.system.storage.get(`char_enabled_${charId}`) !== false;
      if (!isEnabled) return; // 中国角色直接跳过
      ...
      const handleBubbleClick = async (e) => {
        ...
        if (validTranslation) {
          ...
          isTranslating = true;
          if (bubbleEl) bubbleEl.style.opacity = "0.7";
          
          try {
            const result = await translateToChinese(msg.content);
            ctx.system.storage.set(cacheKey, {
              original: msg.content,
              translation: result
            });
            ctx.system.storage.set(stateKey, true);
            updateUI();
          } catch (err) {
            ctx.ui.toast("翻译失败，请检查网络 ⚠️");
          } finally {
            isTranslating = false;
