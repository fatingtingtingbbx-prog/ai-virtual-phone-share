export default {
  ...
  setup(ctx) {
    ...
    // 打开聊天记录详情弹窗（点击卡片时触发，默认滚到底部显示最新消息）
    ...
    async function openForwardModal(sourceSessionId, preselectedId) {
      try {
        ...
        // 只保留「真实角色会话 + 群聊」作为转发目标：
        // 会话对应的人必须在角色列表里存在，不在角色列表里的一律过滤掉（AI 助手等也不会混入）
        const allSessions = (Array.isArray(sessionsRaw) ? sessionsRaw : []).filter((s) => s && s.id);
        const charIdSet = new Set(characters.map((c) => c && c.id).filter(Boolean));
        const sessions = allSessions.filter((s) => {
          if (s.isGroup || s.type === "group" || s.groupId) return true; // 群聊保留
          const cid = s.contactId || s.characterId || s.targetId;
          if (cid && charIdSet.has(cid)) return true; // 单聊：必须是角色列表里的角色
          // 源会话即使不在角色列表里也保留，保证能转出去
          if (s.id === sourceSessionId) return true;
          return false;
        });
        ...
        function resolveMessageSender(m, fallbackTitle) {
          ...
          if (fallbackTitle && fallbackTitle !== "聊天" && fallbackTitle !== "当前聊天" && fallbackTitle !== "群聊" && fallbackTitle !== "未知会话") {
        ...
        const curSessionObj = allSessions.find((s) => s.id === sourceSessionId);
        ...
        ctx.ui.openModal((el, { close }) => {
          ...
          function renderList() {
            ...
            const wasAtBottom = prevScrollHeight - prevScrollTop - listWrap.clientHeight < 30;
            ...
            // 打开时默认滚到最新一条；勾选/操作时保持原位置（原本在底部就继续贴底）
            if (firstRender) {
              firstRender = false;
              // 弹窗此时还没挂载到文档，scrollHeight 还是 0，直接赋值无效；
              // 等挂载完成后再定位：优先滚到长按选中的那条，否则滚到底部（最新消息）。
              requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                  if (preselectedId) {
                    const target = Array.from(listWrap.children).find(
                      (n) => n.dataset && n.dataset.msgId === String(preselectedId)
                    );
                    if (target) {
