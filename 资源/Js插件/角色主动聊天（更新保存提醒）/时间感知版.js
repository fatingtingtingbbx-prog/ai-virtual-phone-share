export default {
  manifest: {
    ...
    apiVersion: 1,
    version: "6.4.0",
    description: "精准感知当前与历史时间！提取30条记忆严守关系边界，像真人一样连发短句。",
    settings: [
      ...
      for (const session of sessions) {
        ...
        try {
          ...
          const recentMsgs = msgs.slice(-30).map(m => {
            const timeStr = formatMsgTime(m.createdAt || m.timestamp);
            return `${timeStr}${m.role === "user" ? "用户" : char.name}: ${m.content}`;
          }).join("\n");
          ...
          // 获取当前真实时间
          const d = new Date();
          const timeString = `${d.getFullYear()}年${d.getMonth()+1}月${d.getDate()}日 星期${['日','一','二','三','四','五','六'][d.getDay()]} ${d.getHours().toString().padStart(2,'0')}:${d.getMinutes().toString().padStart(2,'0')}`;
          
          const prompt = `你是${char.name}，人设：\n${char.persona || char.briefPersona || "无"}\n\n` +
            `【当前真实时间】\n现在是：${timeString}。请在聊天中自然地体现出对当前时间的感知，并**严格计算与历史记录的时间差**。\n\n` +
            `近期聊天记录（共30条，每条前面带有发送时间，用于精准判断当前语境、话题进度和关系状态）：\n${recentMsgs}\n\n` +
            `【必遵核心规则】\n` +
            ...
            `4. 如果你想分多条消息连发（模拟真人连续按回车发送），请务必用 ||| 将每条短句隔开（例如：在吗？|||刚才碰到件超好笑的事|||猜猜是什么）。\n\n` +
